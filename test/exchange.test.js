...
suite('exchange', function(){
    test('buy should add a BUY nockmarcket order', function(done) {
        exchangeData = exchange.buy(40, 100, exchangeData);
        exchangeData.buys.volumes[40].should.eql(100);
        done();
    });

    test('sell should add a SELL nockmarcket order', function(done) {
    ...
    test('sell should produce trades', function(done) {
        ...
        exchangeData.trades[0].should.eql(40);
        exchangeData.trades[0].should.eql(75);
